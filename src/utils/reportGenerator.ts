...
export const generateMonthlyReport = async (
  ...
  autoTable(doc, {
    ...
    body: [
      [
        { content: `RECEITAS\nR$ ${summary.totalIncome.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`, styles: { textColor: [34, 197, 94], fontStyle: 'bold' } },
        { content: `DESPESAS\nR$ ${summary.totalExpense.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`, styles: { textColor: [239, 68, 68], fontStyle: 'bold' } },
        { content: `SALDO\nR$ ${summary.balance.toLocaleString("pt-BR", { minimumFractionDigits: 2 })}`, styles: { textColor: summary.balance >= 0 ? [59, 130, 246] : [239, 68, 68], fontStyle: 'bold' } }
      ]
    ],
    theme: 'plain',
  ...
  categories.slice(0, 8).forEach((cat) => {
    ...
    doc.text(`R$ ${cat.amount.toLocaleString("pt-BR", { minimumFractionDigits: 2 })} (${cat.percentage.toFixed(1)}%)`, 155, chartY);
    chartY += 8;
  });
