...
const Categories = () => {
  ...
  const { deleteTarget, isConfirmOpen, requestDelete, cancelDelete, confirmDelete } =
    useConfirmDelete();
  const { data: categories = [], isLoading } = useSupabaseQuery<Category>(
    "categories",
    "name",
    true,
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={openNew} className="gradient-bg-primary gap-2 text-primary-foreground">
      ...
      {isLoading ? (
        ...
        <div className="space-y-3 py-16 text-center">
          ...
          <p className="text-xs text-muted-foreground">Clique em "Nova Categoria" para criar</p>
        </div>
      ) : (
          ...
          {filtered.map((cat, i) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <Badge
                  variant="outline"
                  className={`mt-0.5 text-[10px] ${cat.type === "income" ? "border-success/30 text-success" : "border-destructive/30 text-destructive"}`}
                >
        ...
        <DialogContent className="glass-card border-border sm:max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  className="h-9 cursor-pointer border-border bg-secondary p-1"
                />
              </div>
            </div>
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-2">
                {ICONS.map((ic) => (
                  <button
                    ...
                    className={`flex h-9 w-9 items-center justify-center rounded-lg text-lg transition-all ${icon === ic ? "bg-primary/10 ring-2 ring-primary" : "bg-secondary hover:bg-secondary/80"}`}
                  >
                    {ic}
                  </button>
                ))}
            ...
            <DialogFooter>
              ...
              <Button
                type="submit"
                disabled={insertMutation.isPending || updateMutation.isPending || !name.trim()}
