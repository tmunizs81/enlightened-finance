...
export function SmartAlerts() {
  ...
  return (
    ...
    >
        ...
        {visibleAlerts.slice(0, 5).map((alert) => {
          ...
          return (
            <motion.div
              ...
              className={`flex items-start gap-3 rounded-lg p-3 ${style.bg} border ${style.border}`}
            >
              <span className="mt-0.5 shrink-0 text-base">{alert.icon}</span>
              <div className="min-w-0 flex-1">
                <p className={`text-xs font-medium ${severityText[alert.severity]}`}>
                  {alert.title}
                </p>
                <p className="mt-0.5 text-[11px] text-muted-foreground">{alert.message}</p>
              </div>
              <button
                onClick={() => handleDismiss(alert.type + alert.title)}
      ...
      {dailyBudget > 0 && (
        <div className="mt-1 flex items-center justify-between rounded-lg border border-primary/15 bg-primary/5 p-3">
          <div>
