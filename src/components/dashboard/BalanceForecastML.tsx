...
export function BalanceForecastML() {
  ...
  const chartData = useMemo(() => {
    ...
    const recentTx = transactions.filter(
      (t) => t.date >= ninetyDaysAgo.toISOString().split("T")[0] && t.status === "paid",
    );

    const totalDays = Math.max(1, Math.ceil((now.getTime() - ninetyDaysAgo.getTime()) / 86400000));
    const avgDailyIncome =
      recentTx.filter((t) => t.type === "income").reduce((s, t) => s + Number(t.amount), 0) /
      totalDays;
    const avgDailyExpense =
      recentTx.filter((t) => t.type === "expense").reduce((s, t) => s + Number(t.amount), 0) /
      totalDays;
    ...
    const dailyNet =
      avgDailyIncome +
      monthlyRecurringIncome / 30 -
      (avgDailyExpense + monthlyRecurringExpense / 30);
    ...
    for (let i = 14; i >= 0; i--) {
      ...
      const dayNet = dayTx.reduce(
        (s, t) => s + (t.type === "income" ? 1 : -1) * Number(t.amount),
        0,
      );
      points.push({
    ...
    for (let i = 1; i <= 90; i++) {
      ...
      if (i % 3 === 0) {
        // sample every 3 days to keep chart clean
        points.push({
          day: d.toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" }),
    ...
    return {
      points,
      projected30: Math.round((currentBalance + dailyNet * 30) * 100) / 100,
      projected60: Math.round((currentBalance + dailyNet * 60) * 100) / 100,
      projected90: Math.round((currentBalance + dailyNet * 90) * 100) / 100,
      currentBalance,
    };
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          <div
            key={p.label}
            className="rounded-lg border border-border/50 bg-secondary/50 p-2 text-center"
          >
            <p className="text-[10px] text-muted-foreground">{p.label}</p>
            <p
              className={`text-xs font-bold ${p.value >= chartData.currentBalance ? "text-success" : "text-destructive"}`}
            >
              R${" "}
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData.points}>
            <XAxis
              ...
              axisLine={false}
              tickLine={false}
              interval={Math.floor(chartData.points.length / 6)}
            />
            <YAxis
              tick={{ fontSize: 9, fill: "hsl(215, 15%, 55%)" }}
              axisLine={false}
      ...
      <p className="text-center text-[10px] text-muted-foreground">
        Linha contínua = real | Projeção baseada em histórico + recorrentes
      </p>
    </motion.div>
  );
}
