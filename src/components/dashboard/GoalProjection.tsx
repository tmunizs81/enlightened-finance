    ...
    return goals
      ...
      .map((goal, idx) => {
        ...
        if (points[points.length - 1]?.value < target) {
          points.push({
            day: estimatedDate.toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" }),
            value: target,
          });
        }
          ...
          <div key={proj.id} className="space-y-2">
            ...
            <div className="h-24">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={proj.points}>
                  <XAxis
                    dataKey="day"
                    tick={{ fontSize: 8, fill: "hsl(215, 15%, 55%)" }}
                    axisLine={false}
                    tickLine={false}
                    interval={Math.floor(proj.points.length / 4)}
                  />
                  <YAxis
                    tick={{ fontSize: 8, fill: "hsl(215, 15%, 55%)" }}
                    axisLine={false}
                    ...
                    domain={[0, Number(proj.target_amount)]}
                  />
                  <Tooltip
                    contentStyle={{
                    ...
                    formatter={(v: number) => [`R$ ${v.toLocaleString("pt-BR")}`, proj.name]}
                  />
                  <ReferenceLine
                    y={Number(proj.target_amount)}
            ...
            {proj.deadline && (
              <p className="text-[10px] text-muted-foreground">
                ...
                {proj.estimatedDate <= new Date(proj.deadline)
                  ? " ✅ No ritmo certo"
                  : " ⚠️ Aumente os aportes"}
              </p>
            )}
          </div>
        ))}
