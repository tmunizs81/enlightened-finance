...
export function BudgetForm({
  ...
  const [notificationsEnabled, setNotificationsEnabled] = useState(
    initialData?.notification_enabled ?? true,
  );
      ...
      <DialogContent className="glass-card border-border sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-foreground">
            {initialData ? "Editar" : "Novo"} Orçamento
          </DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-4 border-t border-border/50 pt-2">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-balance text-[10px] text-muted-foreground">
                  Receber avisos visuais e notificações para esta categoria.
                </p>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <Label className="text-xs font-medium">
                    Gatilho de Alerta ({alertThreshold}%)
                  </Label>
                  <span className="text-[10px] font-bold text-primary">
                    R${" "}
                    {((parseFloat(amount || "0") * alertThreshold) / 100).toLocaleString("pt-BR")}
                  </span>
                </div>
                <Slider
                ...
                <p className="text-[10px] italic text-muted-foreground">
                  Você será avisado quando os gastos atingirem {alertThreshold}% do limite.
                </p>
              </div>
            )}
          </div>
