...
serve(async (req) => {
  ...
  try {
    ...
    if (!requestedUserId) {
      ...
      return new Response(
        JSON.stringify({ users_processed: profiles.length, total_alerts: totalAlerts }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
      );
    ...
    const alertCount = await processUser(
      supabase,
      requestedUserId,
  ...
  for (const budget of budgets) {
    ...
    if (catId) {
      ...
      if (totalSpending > budgetAmount) {
        ...
        alerts.push(
          `⚠️ *Orçamento geral quase no limite!*\nGasto total: R$ ${totalSpending.toFixed(2)} / R$ ${budgetAmount.toFixed(2)} (${pct.toFixed(0)}%)`,
        );
      }
    }
  }
