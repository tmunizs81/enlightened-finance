...
serve(async (req) => {
  ...
  try {
    ...
    if (goals.length === 0) {
      return new Response(
        JSON.stringify({ recommendations: [], summary: "Nenhuma meta cadastrada." }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
      );
    ...
    const monthlyIncome =
      transactions
        ...
        .reduce((s: number, t: any) => s + Number(t.amount), 0) / 2;
    ...
    const goalsSummary = goals
      .map((g: any) => {
        ...
        const daysSinceCreation = Math.max(
          1,
          Math.ceil((now.getTime() - created.getTime()) / 86400000),
        );
        ...
        if (g.deadline) {
          ...
          deadlineStatus =
            daysToComplete > 0 && daysToComplete <= daysLeft
    ...
    const aiResponse = await fetch("https://api.deepseek.com/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "Você é um coach financeiro brasileiro. Responda sempre em português.",
          },
            ...
            function: {
              ...
              parameters: {
                ...
                properties: {
                  summary: {
                    type: "string",
                    description: "Resumo geral da saúde das metas (2-3 frases)",
                  },
                  overall_status: {
                    type: "string",
                  ...
                  recommendations: {
                    ...
                    items: {
                      ...
                      properties: {
                        ...
                        status: {
                          type: "string",
                          enum: ["on_track", "needs_attention", "behind", "completed"],
                        },
                        suggested_monthly: {
                          type: "number",
                          description: "Valor mensal sugerido para aportar",
                        },
                        tip: {
                          type: "string",
                          description: "Dica específica para esta meta (1-2 frases)",
                        },
                        priority: {
                          type: "number",
                          description: "Prioridade 1-5 (1=mais urgente)",
                        },
                      },
                  ...
                  savings_plan: {
                    type: "string",
                    description:
                      "Plano de economia sugerido considerando todas as metas (2-3 frases)",
                  },
                },
