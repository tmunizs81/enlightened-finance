...
serve(async (req) => {
  ...
  try {
    ...
    const monthExpensesPaid = monthTx.filter(
      (t: any) => t.type === "expense" && t.status === "paid",
    );
    ...
    const historicalTx = transactions.filter(
      (t: any) => t.date < firstDay && t.type === "expense" && t.status === "paid",
    );
    ...
    const pendingExpenses = transactions.filter(
      (t: any) => t.type === "expense" && t.status === "pending" && t.date >= firstDay,
    );
    ...
    if (boletosToday.length > 0) {
      alerts.push({
        type: "boleto_today",
        severity: "danger",
        title: `${boletosToday.length} boleto(s) vencem HOJE`,
        message: `Pague antes do fim do dia para evitar multas!`,
        icon: "📄",
      });
    }
    ...
    if (overdue.length > 0) {
      alerts.push({
        ...
        message: `Você tem ${overdue.length} pendência(s) de dias anteriores.`,
        icon: "🚫",
      });
    }
    ...
    for (const g of goals) {
      ...
      if (pct >= 100) {
        alerts.push({
          ...
          message: `Parabéns! Você alcançou R$ ${Number(g.target_amount).toLocaleString("pt-BR")}.`,
          icon: "🏆",
        });
      } else if (pct >= 75) {
        alerts.push({
          ...
          message: `${Math.round(pct)}% concluído. Faltam R$ ${(Number(g.target_amount) - Number(g.current_amount)).toLocaleString("pt-BR")}.`,
          icon: "🎯",
        });
      }
    }
    ...
    if (monthIncome > 0) {
      ...
      if (rate < 0) {
        alerts.push({
          ...
          message: `Seus gastos pagos superaram sua receita neste mês.`,
          icon: "📉",
        });
      }
    }
    ...
    for (const rule of rules) {
      ...
      if (triggered) {
        ...
        alerts.push({
          ...
          message:
            rule.action_message ||
            `Regra ativada${catName ? ` para ${catName}` : ""}: gasto ultrapassou R$ ${Number(rule.condition_amount).toLocaleString("pt-BR")}`,
          icon: "⚡",
        });
      }
    }
